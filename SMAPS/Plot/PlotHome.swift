        ...
        NavigationStack {
            VStack {
                List(plotHead, id: \.id) { plhead in
                    if plhead.status == "1" || plhead.status == "2" {
                        HStack{
                            NavigationLink(destination: {
                                if plhead.status == "1" {
                                    PlotNow(idx: plhead.id)
                                } else {
                                    PlotNowDone(idx: plhead.id)
                                }
                            }){
...
//                    .interactiveDismissDisabled(true)
                ...
                VStack{
                    ...
                    NavigationLink(destination: PlotCreate()){
                        Label("Add Plot", systemImage: "plus")
                            .frame(maxWidth: .infinity, maxHeight: 50)
                            .foregroundColor(Color.white)
                            .bold()
                            .background(Color.green)
